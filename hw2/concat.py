...
def beam_search(model, X_test,target_seq, prob,  path, lens):
    ...
    global decode_seq
    node = 2
    y_pred = model.predict([Xtest,target_seq])
...
with open('models/'+LOAD_MODEL_NUM+"/result_bs" + str(LOAD_MODEL_NUM)+".csv", 'w') as file:
